...
- [ ] TASK-010: Разработка пользовательского интерфейса (Status: IN_PROGRESS, Priority: Medium, Note: Полноценный UI выведен за рамки ВКР, фокус на API и Swagger -> РЕШЕНО РЕАЛИЗОВАТЬ МИНИМАЛЬНЫЙ UI ДЛЯ ДЕМОНСТРАЦИИ)
...
1. Определение ключевых сценариев для демонстрации в UI:
   - [ ] Отображение информации об активной ML-модели (имя, версия, метрики).
   - [ ] Отображение результатов классификации (таблица клиентов и их сегменты лояльности) на основе последнего запуска или демонстрационного набора данных.
   - [ ] Отображение распределения клиентов по сегментам лояльности с визуализацией (например, круговая диаграмма или гистограмма, показывающая процент и количество клиентов в каждом сегменте).
   - [ ] (Опционально, если реализуемо в сжатые сроки) Возможность загрузки небольшого CSV-файла для классификации и отображения результатов.
2. Проектирование FastAPI эндпоинтов для UI:
   - [ ] Эндпоинт для главной страницы UI.
   - [ ] Эндпоинт для получения данных об активной модели.
   - [ ] Эндпоинт для получения результатов классификации.
   - [ ] Эндпоинт для получения статистики по сегментам.
   - [ ] (Опционально) Эндпоинт для загрузки файла и запуска классификации.
3. Создание HTML-шаблонов (Jinja2) для UI:
   - [ ] Базовый шаблон (шапка, подвал, навигация если нужна).
   - [ ] Шаблон для главной страницы (отображение информации о модели, статистика).
   - [ ] Шаблон для страницы с результатами классификации.
   - [ ] (Опционально) Шаблон для формы загрузки файла.
4. Реализация логики в FastAPI для обслуживания UI:
   - [ ] Подключение Jinja2Templates.
   - [ ] Функции-обработчики для эндпоинтов, передающие данные в шаблоны.
   - [ ] Логика для чтения информации о модели (из `output/models/best_model.pkl` или `trained_models_bundle.pkl`).
   - [ ] Логика для чтения/форматирования результатов классификации (из сохраненных файлов или через запуск части пайплайна).
   - [ ] (Опционально) Логика для обработки загруженного файла и запуска классификации.
